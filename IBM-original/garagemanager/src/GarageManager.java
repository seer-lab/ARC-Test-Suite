...
public class GarageManager
 {
    ...
    private String strBugProbability = "littile";
     private int iBugProbability = 250; // DK: Was 200
     private int printedCard = 0;
   ...
   protected GarageStatus status;
     ...
     public static void main(String[] args)
     {
         ...
         bos.GetParametersFromUser(args);  // Process input params
         bos.TakeWorkersFromAgency();  // Get the worker names
     ...
     public void TakeWorkersFromAgency()
     {
        ...
        if(!GetWorkersNames(fileInput))
            HireForeignWorkers();   // Use default names
     ...
     public void GiveTasksToWorkers()
     {
         ...
         for(int i = 0; i < NUMBER_OF_WORKERS; i++)
             // Get a hash of the full or substring of name
             // that is, a task number (not a task)
             taskNumber[i] = SetTaskToWorker(workersNames[i]);
    ...
    // Give worker a task number (not a task)
    ...
    class GarageWorker extends Thread
    {
        ...
        public void GoToWork()
        {
            switch(taskNumber % 8)
            {
                ...
                case 1:
                {
                    int bosKindness = (int)(Math.random() * 100);
                    if(bosKindness > 90)
        ...
        {
            return (printedCard == NUMBER_OF_WORKERS);
        }
        ...
        public  void PrintCard()
        {
            ...
            // This is done to recognize that a bug accured.
