...
public class Reporter
{
    ...
    public boolean Report(String output, int [] originalArray) throws IOException
    {
        try
        {
            ...
            boolean result = IndicateBug(originalArray);// writing the bug to the output file
            out.close();
            return result;
        }
        catch(IOException e)
    ...
    private boolean /*String*/ IndicateBug(int [] originalArray)
    {
        ...
        for(int i = 0;i < arrSize;++i)
        {
            ...
            if(j == arrSize) // in case the number in the original array does not exist in the printed array
                return true;//(",(Bug Found: the number " + originalArray[i] + ", exists in the original array," +
        ...
        for(int i = 0;i < arrSize - 1;++i)
        {
            if(printedArray[i] > printedArray[i + 1])
                return true;//(",(Bug Found: The number at place " + (i + 1) + '(' + printedArray[i] + ')'
            //+ ", is bigger then the number at place " + (i + 2) + '(' + printedArray[i + 1] + ").)>");
        }
        return false; //",No bugs found>";
    }
 }
