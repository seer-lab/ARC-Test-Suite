...
public class Reporter
{
     ...
     * @throws IOException
     */
    public boolean Report(String output, int [] originalArray) throws IOException
    {
        try
        {
            ...
            System.out.print("<BubbleSort Program,");
            ...
            // DK
            boolean OperatedCorrectly = IndicateBug(originalArray);// writing the bug to the output file
            out.close();
    ...
    // DK: Modified to return a boolean
    private boolean /*String*/ IndicateBug(int [] originalArray)
    {
        ...
        for(int i = 0;i < arrSize;++i)
        {
            ...
            {
                try {
              System.out.print(" (Bug Found: the number " + originalArray[i] + ", exists in the original array," +
                " but does not exist in the printed array)>\n");
              out.write(" (Bug Found: the number " + originalArray[i] + ", exists in the original array," +
                " but does not exist in the printed array)>\n");
              return false;}
                      catch(IOException e)
        {
        ...
        for(int i = 0;i < arrSize - 1;++i)
        {
            ...
            {
                try {
              System.out.print(" (Bug Found: The number at place " + (i + 1) + '(' + printedArray[i] + ')'
                + ", is bigger then the number at place " + (i + 2) + '(' + printedArray[i + 1] + ").)>\n");
              out.write(" (Bug Found: The number at place " + (i + 1) + '(' + printedArray[i] + ')'
                + ", is bigger then the number at place " + (i + 2) + '(' + printedArray[i + 1] + ").)>\n");
              return false;}
                        catch(IOException e)
        {
