...
public class SoftWareVerificationHW
{
    ...
    static public boolean main(String args[])
    {
        // getting concurency
        if(args.length > 1) // 2nd argument to main should be the concurency
        {
            concurency = args[1]; // getting the concurency - 2nd parameter
            if(concurency.equals(hi_concurency))
                    sync = hi_sync; // DK: Fixed
            else if(concurency.equals(mid_concurency))
                sync = mid_sync;
            else if (concurency.equals(low_concurency))// in case of hi concurency or need for default value
                sync = low_sync; // setting default value
            else
            {
              // DK: Fixing sync setting and error message   
              // in case no legal value was entered
        ...
        try
        {
            ...
            // DK: Returns OperatedCorrectly
            return Reporter.GetInstance(bsOutput, array.length).Report(rOutput, array);// creatinr the report
        }
        catch( IOException e)
        { 
          return false;
